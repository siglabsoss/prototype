...
%load clock_comb_120k.mat
load tuesdays_transmission.mat
%load rawdata_tuesday.mat
load tuesdays_rx.mat
...
xcorrdetect = 4.65; %max peak to rms ratio for clock comb xcorr search
windowtype = @triang;
...
goodsets2 = find(samplesoffsetxcorr > 0); % filter out partial coverage
%reduce to just the good datasets
...
for k = 1:1:numdatasets
    subplot(numdatasets,1,k)
    aligneddataxcorr(:,k) = [freqaligneddataxcorr2(samplesoffsetxcorr2(k):end,k);zeros([samplesoffsetxcorr2(k)-1 1])]./exp(i*(recoveredphasexcorr2(k)));
    subplot(numdatasets,1,k)
