...
clock_comb = clock_comb195k;
 ...
 rnoisydata = clock_comb195k;
 timestamp = 0:srate:(length(rnoisydata)-1)*srate;
  
freqshift=25;
timeshift = 100*srate;
phaseshift = pi/2;

rnoisydata = rnoisydata.*exp(i*2*pi*timestamp*freqshift)';
rnoisydata = rnoisydata*exp(i*2*pi*phaseshift);
rnoisydata = [zeros([timeshift/srate 1]);rnoisydata(1:end-timeshift/srate)];
...
rnoisydata = downsample(rnoisydata,downsample_factor);
clock_comb = downsample(clock_comb,downsample_factor);
srate = downsample_factor*srate;
samples_per_bit_at_fs = samples_per_bit_at_fs/downsample_factor;
datalength = length(rnoisydata(:,1));
timestamp = 0:srate:(datalength-1)*srate;
[aligned_data retro_data retrostart retroend fxcorrsnr goodsets freqoffset phaseoffset samplesoffset] = retrocorrelator_octave_size(rnoisydata,srate,clock_comb,detect_threshold,clock_comb,fsearchwindow_low,fsearchwindow_hi,retro_go,weighting_factor);
...
figure
plot(real(aligned_data*(ones([length(aligned_data(1,:)) 1])))) 
title('Coherent Sum')
hold on
plot(real(clock_comb),'r')
...
for k = 1:1:length(aligned_data(1,:))
    BER_single(k) = 1-sum(o_cpm_demod(aligned_data(:,k),srate,samples_per_bit_at_fs,patternvec,1) == ideal_bits)/length(ideal_bits);
end
